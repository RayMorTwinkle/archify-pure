...
function longDetour(profile = 'showcase') {
  return {
    ...
    meta: { title: 'Long detour', quality_profile: profile },
    components: [
...
test('an unnecessary control corridor is diagnosed inside a large content envelope', t => {
  const diagram = {
    schema_version: 1,
    diagram_type: 'architecture',
    meta: { title: 'Internal empty corridor', quality_profile: 'showcase' },
    components: [
...
test('an explicit route around an opaque obstacle is not reported as an excessive detour', t => {
  const diagram = {
    schema_version: 1,
    diagram_type: 'architecture',
    meta: { title: 'Obstacle route', quality_profile: 'showcase' },
    components: [
      component('source', [60, 100]),
...
test('a related shared outer corridor is treated as an intentional bus', t => {
  const diagram = {
    schema_version: 1,
    diagram_type: 'architecture',
    meta: { title: 'Shared bus', quality_profile: 'showcase' },
    components: [
      component('source', [60, 100]),
...
test('a direct relationship crossing a region boundary is not a detour', t => {
  const diagram = {
    schema_version: 1,
    diagram_type: 'architecture',
    meta: { title: 'Boundary crossing', quality_profile: 'showcase' },
