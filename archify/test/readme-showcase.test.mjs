...
const readmeFiles = ['README.md', 'README_EN.md', 'README_ZH.md'];
...
test('every README link resolves locally or to traced proof provenance', () => {
  // Provenance for a mapped third-party repository is the only remote target allowed.
  const allowedRemote = /^https:\/\/github\.com\/mco-org\/mco$/;
  for (const filename of readmeFiles) {
    const readme = read(filename);
    const targets = [...readme.matchAll(/\]\(([^)\s]+)\)/g)]
      .map((match) => match[1])
      .concat([...readme.matchAll(/src="([^"]+)"/g)].map((match) => match[1]))
      .filter((target) => !target.startsWith('#'));

    for (const target of targets) {
      assert.ok(!/^https?:\/\//.test(target) || allowedRemote.test(target),
        `${filename}: remote link ${target} leaves the machine`);
      if (/^https?:\/\//.test(target)) continue;
      const relative = target.split('#')[0].split('?')[0];
      if (!relative || relative.startsWith('./')) continue;
      assert.ok(fs.existsSync(path.join(repoRoot, relative)), `${filename}: broken local link ${target}`);
    }
  }
});

test('the READMEs carry no sponsor placement, referral tag, or badge request', () => {
  for (const filename of readmeFiles) {
    const readme = read(filename);
    assert.doesNotMatch(readme, /^## .*Sponsor/im, `${filename}: sponsor section returned`);
    assert.doesNotMatch(readme, /^## .*赞助/m, `${filename}: sponsor section returned`);
    assert.doesNotMatch(readme, /utm_[a-z]+|[?&](ref|rc)=/i, `${filename}: referral tracking parameter`);
    assert.doesNotMatch(readme, /mailto:/i, `${filename}: contact call to action`);
    assert.doesNotMatch(readme, /img\.shields\.io|trendshift\.io|badgen\.net|apispace|opencollective|patreon|ko-fi/i,
      `${filename}: badge or sponsorship service referenced`);
  }
});

test('all README languages keep the product hero and the verified animated proof', () => {
  for (const filename of readmeFiles) {
    const readme = read(filename);
...
test('README demos use checked-in captures and local deep links', () => {
  const demos = [
    {
...
test('README stays scannable without deleting the visual proof set', () => {
  ...
  const intro = english.slice(0, english.indexOf('**Current development version'))
    || english.slice(0, english.indexOf('![Archify product preview'));
